...
try:
    ...
    class EmergencySymbolManager:
        def get_safe_symbol(self, symbol):
            ...
            )  # Default to best performing symbol
...
class MT5TradingInterface:
            ...
            if all_symbols:
                ...
                for symbol in all_symbols:
                    ...
                    if any(
                        ...
                        for keyword in ["volatility", "boom", "crash", "vol", "r_"]
                    ):
                ...
                if deriv_symbols:
                    logger.info(
                        f"✅ Found {len(deriv_symbols)} Deriv symbols: {deriv_symbols[:10]}"
                    )
    ...
    async def get_current_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            if symbol_info is None:
                logger.error(
                    f"❌ Symbol {effective_symbol} not found in MT5 - checking alternatives"
                )
                ...
                for alt_symbol in alternatives:
                    ...
                    logger.error(
                        f"❌ No valid symbols found for {symbol}. Checking available symbols..."
                    )
                    ...
                    if symbols:
                        volatility_symbols = [
                            s.name
                            for s in symbols
                            if "vol" in s.name.lower() or "index" in s.name.lower()
                        ]
                        logger.info(
            ...
            for attempt in range(5):  # Increased attempts
                ...
                if tick is not None:
                    ...
                    logger.info(
                        f"🔍 Tick data for {effective_symbol}: bid={bid_price}, ask={ask_price}, last={last_price}"
                    )
            ...
            logger.error(
                f"❌ Failed to get price for {symbol} ({effective_symbol}) after 3 attempts"
            )
    ...
    async def validate_lot_size(self, symbol: str, volume: float) -> Tuple[bool, float]:
        ...
        try:
            ...
            # Final validation
            is_valid = (
                min_vol <= adjusted_volume <= max_vol
                and abs(adjusted_volume - round(adjusted_volume / step) * step) < 0.0001
            )

            logger.debug(
                f"🔍 Lot validation for {symbol}: {volume} → {adjusted_volume} (valid: {is_valid})"
            )
        ...
        try:
            ...
            for key, default_value in defaults.items():
                if key not in symbol_info or symbol_info[key] is None:
                    logger.warning(
                        f"⚠️ Missing or invalid {key} for {symbol}, using default: {default_value}"
                    )
                ...
                try:
                    if key in ["volume_min", "volume_max", "volume_step"]:
                        if float(symbol_info[key]) <= 0:
                            logger.warning(
                                f"⚠️ Invalid {key} value {symbol_info[key]} for {symbol}, using default: {default_value}"
                            )
                            symbol_info[key] = default_value
                except (ValueError, TypeError):
                    logger.warning(
                        f"⚠️ Non-numeric {key} for {symbol}, using default: {default_value}"
                    )
                    symbol_info[key] = default_value
    ...
    async def calculate_valid_lot_size(self, symbol: str, desired_amount: float) -> float:
        ...
        try:
            ...
            logger.info(
                f"📊 MT5 {effective_symbol} limits: min={volume_min}, max={volume_max}, step={volume_step}"
            )
            ...
            logger.info(
                f"💎 BULLETPROOF lot calculation: ${desired_amount} → {final_lot:.3f} lots "
    ...
    async def place_trade(
        self, action: str, symbol: str, amount: float
        ...
        try:
            ...
            if lot_size < volume_min or lot_size > volume_max:
                logger.error(
                    f"❌ CRITICAL: Lot size {lot_size} outside MT5 limits [{volume_min}, {volume_max}]"
                )
            ...
            logger.info(
                f"💎 BULLETPROOF lot size: {lot_size:.3f} for ${amount} on {effective_symbol}"
            )
            ...
            if (
                ...
                or "Boom" in effective_symbol
                or "Crash" in effective_symbol
            ):
            ...
            for i, filling_type in enumerate(filling_types):
                try:
                    ...
                    if result is None:
                        ...
                        logger.error(
                            f"❌ MT5 order_send returned None (attempt {i+1}) - Last Error: {last_error}"
                        )
                    ...
                    if result.retcode == mt5.TRADE_RETCODE_DONE:
                        ...
                        logger.info(
                            f"✅ BULLETPROOF MT5 SUCCESS: {action} {effective_symbol} (Ticket: {trade_ticket})"
                        )
                        ...
                        logger.warning(
                            f"⚠️ Attempt {i+1} failed: {result.retcode} - {error_msg} (Filling: {filling_type})"
                        )
    ...
    async def _simulate_trade(
        self, action: str, symbol: str, amount: float, price: float
    ) -> Dict[str, Any]:
    ...
    async def update_available_symbols(self) -> None:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if any(
                    keyword in symbol_name.lower()
                    for keyword in ["volatility", "boom", "crash", "step", "jump"]
                ):
    ...
    def calculate_lot_size(self, symbol: str, risk_amount: float) -> float:
        ...
        try:
            ...
            if symbol_info is None:
                logger.warning(
                    f"⚠️ Cannot get symbol info for {symbol} (MT5: {effective_symbol}), using default lot size"
                )
            ...
            logger.info(
                f"💎 Calculated lot size: {lot_size} for ${risk_amount:.2f} on {effective_symbol} "
    ...
    async def close_position(self, ticket: int) -> bool:
        ...
        try:
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                logger.warning(
                    f"⚠️ Close failed (retcode: {result.retcode}), trying alternative method"
                )
...
class MT5TradingBot:
    ...
    async def place_trade_mt5(
        ...
        try:
            ...
            # Place order
            if action == "BUY":
                result = await self.mt5.place_buy_order(
                    symbol, lot_size, comment=f"AI Bot {action}"
                )
            else:  # SELL
                result = await self.mt5.place_sell_order(
                    symbol, lot_size, comment=f"AI Bot {action}"
                )
    ...
    async def _validate_symbol_ready(self, symbol: str) -> bool:
        ...
        try:
            ...
            if symbol_info.bid <= 0 or symbol_info.ask <= 0:
                logger.error(
                    f"❌ Invalid prices for symbol {symbol}: bid={symbol_info.bid}, ask={symbol_info.ask}"
                )
                return False
    ...
    async def _try_alternative_order_method(
        self, request: Dict[str, Any], symbol: str
    ) -> Optional[Any]:
