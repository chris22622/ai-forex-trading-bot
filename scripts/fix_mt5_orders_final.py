...
def fix_mt5_order_issues():
    ...
    if result is None:
        ...
        print(f"❌ order_send STILL returns None!")
        print(f"❌ Error: {error}")
        ...
        if result is None:
            ...
            print(f"   1. Check MT5 terminal is connected to broker server")
            print(f"   2. Verify account has live trading permissions")
            print(f"   3. Ensure broker allows automated trading")
            print(f"   4. Check if demo account requires special settings")
            return False
    else:
...
def apply_mt5_fixes():
    ...
    print(f"🎯 NEXT STEPS:")
    print(f"   1. Ensure MetaTrader 5 terminal is running")
    print(f"   2. Connect to your broker's trading server")
    print(f"   3. Verify account login and trading permissions")
    print(f"   4. Enable auto-trading in MT5 terminal")
    print(f"   5. Restart the trading bot")
