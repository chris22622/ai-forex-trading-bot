...
def comprehensive_index_check():
    ...
    if len(active_indices) >= 5:
        print("   ✅ Excellent coverage - Multiple indices available")
        print("   💡 Recommended: Enable multi-symbol trading with top 3-5 indices")
    elif len(active_indices) >= 3:
        print("   ✅ Good coverage - Several indices available")
        print("   💡 Recommended: Enable multi-symbol trading with all active indices")
    elif len(active_indices) >= 1:
        print("   ⚠️ Limited coverage - Few indices available")
        print("   💡 Recommended: Single symbol trading with most active index")
    else:
        print("   ❌ No active indices found - Check market hours or connection")
